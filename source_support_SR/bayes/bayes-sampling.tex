...
\frame{ \frametitle{M-H for logistic regression: R code}
  ...
  ~ \\
  for (b in 2:B)\{ \\ \pause
  \qquad theta.tmp = rmvnorm(1, mean=theta.sample[b-1, ], sigma=Sigma.shift)[1, ] \\ \pause
  ...
  \qquad \qquad theta.sample[b, ] = theta.cur = theta.tmp \\ 
  \qquad \qquad logprior.cur = logprior.tmp; loglik.cur = loglik.tmp \\ \pause
  \qquad \}else\{ \\
...
\frame{ \frametitle{Sanity checks}
  ...
  \paragraph{Setting.} Sample $1.2 \; 10^6$ $\thetabf$, remove first $2 \; 10^5$, extract every 10 \ra $B = 10^5$. \\ ~
